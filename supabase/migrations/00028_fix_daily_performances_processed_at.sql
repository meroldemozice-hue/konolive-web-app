...
BEGIN
  ...
  daily_counts AS (
    SELECT 
      EXTRACT(ISODOW FROM processed_at)::int AS d_idx,
      COUNT(id) AS cnt
    FROM public.verification_requests
    WHERE 
      status IN ('accepted', 'rejected', 'other')
      AND processed_at >= date_trunc('week', timezone('utc', now()))
      AND processed_at < date_trunc('week', timezone('utc', now())) + interval '7 days'
    GROUP BY EXTRACT(ISODOW FROM processed_at)
  )
  SELECT 
    w.name,
    w.idx,
