-- Update public dashboard and internal extra stats to correctly count total received for the day context
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'todayReceived', (SELECT count(*) FROM public.verification_requests WHERE created_at >= current_date OR processed_at >= current_date),
    'accepted', (SELECT count(*) FROM public.verification_requests WHERE status = 'accepted' AND processed_at >= current_date),
    'rejected', (SELECT count(*) FROM public.verification_requests WHERE status = 'rejected' AND processed_at >= current_date),
    'unchanged', (SELECT count(*) FROM public.verification_requests WHERE status = 'unchanged' AND processed_at >= current_date),
    ...
    'autres', (SELECT count(*) FROM public.verification_requests WHERE status = 'other' AND processed_at >= current_date)
  ) INTO v_kpi;
  ...
  FROM (
    SELECT 
      ...
      COUNT(r.id) FILTER (WHERE r.status = 'accepted' AND r.processed_at >= current_date) as accepted,
      COUNT(r.id) FILTER (WHERE r.status = 'rejected' AND r.processed_at >= current_date) as rejected,
      COUNT(r.id) FILTER (WHERE r.status = 'unchanged' AND r.processed_at >= current_date) as unchanged,
      COUNT(r.id) FILTER (WHERE r.status = 'other' AND r.processed_at >= current_date) as autres
    FROM public.verification_requests r
    JOIN public.profiles p ON r.applicant_id = p.id
    WHERE r.created_at >= current_date OR r.processed_at >= current_date
    GROUP BY p.locality
