-- Revert public dashboard and internal extra stats to strictly use created_at as the daily cohort
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'accepted', (SELECT count(*) FROM public.verification_requests WHERE created_at >= current_date AND status = 'accepted'),
    'rejected', (SELECT count(*) FROM public.verification_requests WHERE created_at >= current_date AND status = 'rejected'),
    'unchanged', (SELECT count(*) FROM public.verification_requests WHERE created_at >= current_date AND status = 'unchanged'),
    'pending', (SELECT count(*) FROM public.verification_requests WHERE created_at >= current_date AND status = 'pending'),
    'processing', (SELECT count(*) FROM public.verification_requests WHERE created_at >= current_date AND status = 'processing'),
    'autres', (SELECT count(*) FROM public.verification_requests WHERE created_at >= current_date AND status = 'other')
  ) INTO v_kpi;
  ...
  -- Create cumulative minute-level chart data strictly for today's cohort
  WITH events AS (
